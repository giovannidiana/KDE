...
#batchlist=readLines(paste(args[2],".txt",sep=""));

data12 <- subset(data0,B==Batch,select = c("ADF","ASI"))/1e6;
...
data13 <- subset(data0,B==Batch,select = c("ADF","NSM"))/1e6;
...
data23 <- subset(data0,B==Batch,select = c("ASI","NSM"))/1e6;
