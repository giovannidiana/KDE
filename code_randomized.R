...
data12 <- cbind(ADF=sample(data12$ADF),ASI=sample(data12$ASI))
print(nrow(data12))
...
data13 <- cbind(ADF=sample(data13$ADF),NSM=sample(data13$NSM))
...
data23 <- cbind(ASI=sample(data23$ASI),NSM=sample(data23$NSM))
...
write.table(joint12,paste(OutFolder,"/pdfR12_",GT,"_",Food,"_GS",GS,"_s",seed,".dat",sep=""),col.names=F,row.names=F);
write.table(joint13,paste(OutFolder,"/pdfR13_",GT,"_",Food,"_GS",GS,"_s",seed,".dat",sep=""),col.names=F,row.names=F);
write.table(joint23,paste(OutFolder,"/pdfR23_",GT,"_",Food,"_GS",GS,"_s",seed,".dat",sep=""),col.names=F,row.names=F);
#
